...
ggplot() +
  geom_split_violin(data = pt_pred0, 
                    aes(x = diagnosis, y = value,
                        color = factor(diagnosis))) +
  geom_split_violin(data = pt_pred1, 
                    aes(x = diagnosis, y = value,
                        color = factor(diagnosis))) +
  geom_jitter(data = pitch3,
              aes(x = diagnosis, y = pitchSDLOGz,
                  color = factor(diagnosis)),
              width = -0.2,
             size = 0.5) +
  theme_janco_bar()
...
ggplot() +
  geom_split_violin(data = pt_pred, 
                    aes(x = diagnosis, y = value,
                        color = factor(diagnosis))) +
  geom_split_violin(data = pitch3,
                    aes(x = diagnosis, y = pitchSDLOGz,
                        color = factor(diagnosis)),
                    draw_quantiles = 1)
...
#comparing them 
waic(meta_mod, brm_mod) #the outcome of the meta-analysis does slightly better
```
