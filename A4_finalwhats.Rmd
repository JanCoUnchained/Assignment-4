...
library(pacman)
p_load(tidyverse, readxl, brms, metafor, brmstools, 
       ggunchained, cowplot, ggthemes, tidybayes)
...
#effect size pitch SD (this z-scales I think) #what
...
#can be run instead of the model (while we work). #what
brm_matti <- readRDS("brm_matti.rds", refhook = NULL)

# meta-analysis vuorre inspired. 
brm_matti <- brm(
  yi | se(vi) ~ 1 + (1 | StudyRef), #works, we know the variance. #what
  prior = set_prior("uniform(0, 10)", class = "sd"), 
  data = meta_ready, 
  iter = 10000,
  cores = 4,
  sample_prior = TRUE
  #file = "brm_matti" --> we already have the file #what
) 
...
# model estimates 
brm_mod #could be improved (when is it hacking?..) #what
...
#posterior predictive checks (violin is funky) #what
...
brms::marginal_effects(brm_mod) #big effect (presumably, the scale is fucked..) #what
```

TASK 3 (continued)
more plots.
THIS IS THE ONLY THING STILL HOLDING US BACK! #what
...
#sanity check (just plot() since pairs() is too messy) #what
...
#call the plot
p3 #not sure about the vline() #what
...
Notice that they are identical now (actually the sceptic is infinitesimally better). This might be because I hacked the priors more. Very hard to set priors after so many conversions. #what
...
```{r}
post_mod <- posterior_samples(brm_mod)

tibble_post_mod <- post_mod %>% 
  gather() %>% 
  group_by(key) %>% 
  mean_hdi(value, .width = .99)

post_met <- posterior_samples(brm_meta)

tibble_post_met <- post_met %>%
  gather() %>%
  group_by(key) %>%
  mean_hdi(value, .width = .99)

#transformation own priors
head(tibble_post_mod) 
stuff <- c("value", "lower", "upper")
value <- c(-0.402, -4.79e-1, -0.323)
df <- data.frame(stuff, value)
df_mod <- transfer(df, "value")

#transformations meta priors 
head(tibble_post_met)
value <- c(-0.419, -4.94e-1, -0.345)
df1 <- data.frame(stuff, value)
df1_met <- transfer(df1, "value")

#malte code 
#default: groupings from data (what we did already)

#average actor
d.pred <- distinct(pitch3, diagnosis, studynr, ID, trial)
avg = predict(brm_mod, re_formula = NA, newdata = d.pred, summary = FALSE)

#marginal actor
mar = predict(brm_mod, newdata = d.pred, allow_new_levels = TRUE, sample_new_levels = "gaussian", summary = FALSE)

#IMPORTANT!! plots of this is to be found in "randomplots.Rmd"
#run that code & then this can be run. 
#NB: there is a difference in the violins even though it looks very similar.  #what explain maybe
plot_grid(g, g1, g2) 
